...
class MLP_geometry(nn.Module):
    """Decoder. Point coordinates not only used in sampling the feature grids,
    but also as MLP input.
    ...
    Args:
        ...
        concat_feature (bool): whether to get feature from
    ...
    def __init__(self,
        ...
        self.skips = skips
        self.weighting = pointcloud_nn_weighting
        self.use_dynamic_radius = use_dynamic_radius
        self.min_nn_num = pointcloud_min_nn_num
        self.N_surface = rendering_n_surface
        ...
        self.pts_linears = nn.ModuleList([
            ...
            DenseLayer(hidden_size, hidden_size, activation='relu') if i not in
            self.skips else DenseLayer(
                hidden_size + embedding_input, hidden_size, activation='relu')
            for i in range(n_blocks - 1)
        ])

        self.output_linear = DenseLayer(hidden_size, 1, activation='relu')
    ...
    def get_feature_at_pos(self,
        ...
        device = p.device
        cloud_pos = torch.tensor(npc._cloud_pos, device=device)
        p = p.reshape(-1, 3)
        D, I, neighbor_num = npc.find_neighbors_faiss(
            p.detach().clone(), step='query', dynamic_radius=dynamic_r_query)
        # faiss returns "D" in the form of squared distances.
        # Thus we compare D to the squared radius
        radius_query_bound = (npc.get_radius_query()**2
                              if not self.use_dynamic_radius else
                              dynamic_r_query.reshape(-1, 1)**2)
        if is_tracker:
            # re-calculate D to propagate gradients to the camera extrinsics
            nn_num = D.shape[1]
            D = torch.sum(torch.square(cloud_pos[I] - p.reshape(-1, 1, 3)),
                          dim=-1)
        ...
        # select neighbors within range, then interpolate feature
        ...
        # rays that have no neighbors are thus rendered with
        # random feature vectors for depth
        c[~has_neighbors] = torch.zeros([self.c_dim],
                                        device=device).normal_(mean=0,
                                                               std=0.01)
    ...
    def forward(self,
        ...
        Args:
            ...
            Contains the optimizable parameters during mapping
            pts_num (int, optional): sampled pts num along each ray.
            Defaults to N_surface.
            is_tracker (bool, optional): whether called by tracker.
            Defaults to False.
            cloud_pos (tensor, optional): point cloud position.
            pts_views_d (tensor): viweing directions
            dynamic_r_query (tensor, optional): if enabled dynamic
            radius, query radius for every pixel will be different.

        Returns:
            out (tensor): occupancies for the points p
            valid_ray_mask (bool): boolean tensor. True if at least half
            of all points along the ray have neighbors
            has_neighbors (bool): boolean tensor. False if at least two
            neighbors were not found for the point in question
        """

        npc_geo_feats = npc.get_geo_feats()
        c, has_neighbors = self.get_feature_at_pos(
            npc, p, npc_geo_feats, is_tracker,
            dynamic_r_query=dynamic_r_query)  # get (N,c_dim), e.g. (N,32)

        # ray is not close to the current npc, choose bar here
        # a ray is considered valid if at least half of all points along
        # the ray have neighbors.
        valid_ray_mask = ~(torch.sum(has_neighbors.view(-1, pts_num), 1) <
                           int(self.N_surface / 2 + 1))
        ...
        for i, l in enumerate(self.pts_linears):
            ...
            if self.c_dim != 0:
                ...
                c_i = self.fc_c[i](c)
                h = h + c_i
                # so for hidden layers in the decoder, its input comes from
                # both its feature and embedded location.
            if i in self.skips:
...
class MLP_color(nn.Module):
    ...
    def __init__(self,
        ...
        self.N_surface = rendering_n_surface
        self.use_view_direction = use_view_direction
        self.encode_rel_pos_in_col = model_encode_rel_pos_in_col
        self.encode_exposure = model_encode_exposure
        self.encode_viewd = model_encode_viewd
        ...
        if self.encode_exposure:
            self.mlp_exposure = MLP_exposure(model_exposure_dim, hidden_size)
        ...
        self.output_linear = DenseLayer(hidden_size, 3, activation='linear')
    ...
    def get_feature_at_pos(self,
        ...
        )**2 if not self.use_dynamic_radius else dynamic_r_query.reshape(
            -1, 1)**2
        if is_tracker:
            # re-calculate D to propagate gradients to the camera extrinsics
            nn_num = D.shape[1]
        ...
        weights = F.normalize(weights, p=1, dim=1).unsqueeze(-1)
        # use fixed num of nearest nn
        # select neighbors within range, then interpolate feature by
        ...
        if self.encode_rel_pos_in_col:
            ...
            neighbor_feats = torch.cat([
                embedding_rel_pos.reshape(
                    neighbor_pos.shape[0], -1,
                    self.embedder_rel_pos.mapping_size * 2), neighbor_feats
            ],
                                       dim=-1)  # (N, nn_num, 40+c_dim)
        ...
        c = c.sum(axis=1).reshape(-1, self.c_dim)
        # points with no neighbors are given a random feature vector
        # rays that have no neighbors are thus rendered with random
        # feature vectors for color
        c[~has_neighbors] = torch.zeros([self.c_dim],
                                        device=device).normal_(mean=0,
                                                               std=0.01)
    ...
    def forward(self,
        ...
        Args:
            ...
            cloud_pos (tensor, optional): point cloud position,
                               used when called by tracker to re-calculate D.
            pts_views_d (tensor): viweing directions
            dynamic_r_query (tensor, optional): if enabled dynamic radius,
                     query radius for every pixel will be different.
            exposure_feat (tensor): exposure feature vector.
                Needs to be the same for all points in the batch.

        Returns:
            predicted colors for points p
        """
        npc_col_feats = npc.col_feats

        c, _ = self.get_feature_at_pos(npc,
                                       p,
                                       npc_col_feats,
                                       is_tracker,
        ...
        if self.use_view_direction:
            ...
            embedded_input = torch.cat([embedded_pts, embedded_views_d], -1)
        h = embedded_input
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i](h)
            h = self.actvn(h)
            if self.c_dim != 0:
                ...
                c_i = self.fc_c[i](c)
                h = h + c_i
                # so for hidden layers in the decoder, its input comes
                # from both its feature and embedded location.
            if i in self.skips:
                h = torch.cat([embedded_input, h], -1)
        out = self.output_linear(h)
        if self.encode_exposure:
            if exposure_feat is not None:
                ...
                rot, trans = affine_tensor[:9].reshape(3,
                                                       3), affine_tensor[-3:]
                out = torch.matmul(out, rot) + trans
                out = torch.sigmoid(out)
            else:
                # apply exposure compensation outside
                # "self.renderer.render_batch_ray" call in mapper
                # this is done when multiple exposure feature vectors
                # are needed for different rays during mapping.
                # Each keyframe has its own exposure feature vector,
                # while the forward function of the MLP_color class
                # assumes that all rays have the same exposure feature
                # vector.
                return out
        else:
            out = torch.sigmoid(out)
...
class POINT(nn.Module):
    ...
    def __init__(self,
        ...
        self.geo_decoder = MLP_geometry(
            use_dynamic_radius=use_dynamic_radius,
            pointcloud_min_nn_num=pointcloud_min_nn_num,
            pointcloud_nn_weighting=pointcloud_nn_weighting,
            rendering_n_surface=rendering_n_surface,
            c_dim=c_dim,
            skips=[2],
            n_blocks=5,
            hidden_size=32,
            pos_embedding_method=pos_embedding_method)
        self.color_decoder = MLP_color(
            use_dynamic_radius=use_dynamic_radius,
            pointcloud_min_nn_num=pointcloud_min_nn_num,
            pointcloud_nn_weighting=pointcloud_nn_weighting,
            rendering_n_surface=rendering_n_surface,
            model_encode_rel_pos_in_col=model_encode_rel_pos_in_col,
            model_encode_exposure=model_encode_exposure,
            model_encode_viewd=model_encode_viewd,
            model_exposure_dim=model_exposure_dim,
            c_dim=c_dim,
            skips=[2],
            n_blocks=5,
    ...
    def forward(self,
        ...
        Args:
            ...
            Contains the optimizable parameters during mapping
            pts_num (int): number of points in sampled in each ray,
            used only by geo_decoder.
            is_tracker (bool): whether called by tracker.
            cloud_pos (tensor): (N,3)
            pts_views_d (tensor): used if color decoder encodes
            viewing directions.
        ...
        Returns:
            ...
            ray_mask (tensor): boolean tensor. True if at least half of
            all points along the ray have neighbors
            point_mask (tensor): boolean tensor. False if at least two
            neighbors were not found for the point in question
        """
        if stage == 'geometry':
            geo_occ, ray_mask, point_mask = self.geo_decoder(
                p,
                npc,
                pts_num=pts_num,
                is_tracker=is_tracker,
                dynamic_r_query=dynamic_r_query)
            raw = torch.zeros(geo_occ.shape[0], 4, dtype=torch.float)
            raw[..., -1] = geo_occ
