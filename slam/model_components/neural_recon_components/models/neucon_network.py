...
from slam.model_components.neural_recon_components.ops.back_project import \
    back_project
from slam.model_components.neural_recon_components.ops.generate_grids import \
    generate_grid
from slam.model_components.utils import apply_log_transform
...
class NeuConNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        ch_in = [
            80 * alpha + 1, 96 + 40 * alpha + 2 + 1, 48 + 24 * alpha + 2 + 1,
            24 + 24 + 2 + 1
        ]
        channels = [96, 48, 24]
    ...
    def get_target(self, coords, inputs, scale):
        ...
        with torch.no_grad():
            ...
            tsdf_target = tsdf_target[coords_down[:, 0], coords_down[:, 1],
                                      coords_down[:, 2], coords_down[:, 3]]
            occ_target = occ_target[coords_down[:, 0], coords_down[:, 1],
                                    coords_down[:, 2], coords_down[:, 3]]
    ...
    def upsample(self, pre_feat, pre_coords, interval, num=8):
        ...
        :param pre_coords: (Tensor), coordinates from last level, (N, 4)
                           (4 : Batch ind, x, y, z)
        :param interval: interval of voxels, interval = scale ** 2
        :param num: 1 -> 8
        :return: up_feat : (Tensor), upsampled features, (N*8, C)
        :return: up_coords: (N*8, 4), upsampled coordinates,
                            (4 : Batch ind, x, y, z)
        '''
        with torch.no_grad():
    ...
    def forward(self, features, inputs, outputs):
        ...
        :param features: list: features for each image:
                  eg. list[0] : pyramid features for image0 :
        ...
        for i in range(self.cfg.N_LAYER):
            ...
            if i == 0:
                ...
                for b in range(bs):
                    up_coords.append(
                        torch.cat([
                            torch.ones(1, coords.shape[-1]).to(coords.device) *
                            b, coords
                        ]))
                up_coords = torch.cat(up_coords,
                                      dim=1).permute(1, 0).contiguous()
            else:
                # upsample coords
                up_feat, up_coords = self.upsample(pre_feat, pre_coords,
                                                   interval)

            # back project
            feats = torch.stack([feat[scale] for feat in features])
            KRcam = inputs['proj_matrices'][:, :,
                                            scale].permute(1, 0, 2,
                                                           3).contiguous()
            volume, count = back_project(up_coords,
            ...
            for b in range(bs):
                ...
                coords_batch = coords_batch * self.cfg.VOXEL_SIZE + inputs[
                    'vol_origin_partial'][b].float()
                coords_batch = torch.cat(
                    (coords_batch, torch.ones_like(coords_batch[:, :1])),
                    dim=1)
                coords_batch = coords_batch @ inputs[
                    'world_to_aligned_camera'][b, :3, :].permute(
                        1, 0).contiguous()
            ...
            if self.training and num > self.cfg.TRAIN_NUM_SAMPLE[i] * bs:
                choice = np.random.choice(num,
                                          num -
                                          self.cfg.TRAIN_NUM_SAMPLE[i] * bs,
                                          replace=False)
            ...
            for b in range(bs):
                ...
                if len(batch_ind) == 0:
                    print('no valid points: scale {}, batch {}'.format(i, b))
                    return outputs, loss_dict
    ...
    def compute_loss(tsdf,
        ...
        occ_loss = F.binary_cross_entropy_with_logits(occ,
                                                      occ_target.float(),
                                                      pos_weight=w_for_1)
