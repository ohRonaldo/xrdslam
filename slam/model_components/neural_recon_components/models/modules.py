...
from ..ops.torchsparse_utils import (initial_voxelize, point_to_voxel,
                                     voxel_to_point)
...
class SPVCNN(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if 'press' in kwargs and 'vres' in kwargs:
    ...
    def forward(self, z):
        ...
        x0 = initial_voxelize(z, self.press, self.vres)
...
class SConv3d(nn.Module):
    ...
    def __init__(self, inc, outc, press, vres, ks=3, stride=1, dilation=1):
        ...
        self.net = spnn.Conv3d(inc,
                               ...
                               dilation=dilation,
                               stride=stride)
        self.point_transforms = nn.Sequential(nn.Linear(inc, outc), )
        self.press = press
        self.vres = vres

    def forward(self, z):
        x = initial_voxelize(z, self.press, self.vres)
        x = self.net(x)
...
class ConvGRU(nn.Module):
    ...
    def __init__(self, hidden_dim=128, input_dim=192 + 128, press=1, vres=1):
        ...
        self.convz = SConv3d(hidden_dim + input_dim, hidden_dim, press, vres,
                             3)
        self.convr = SConv3d(hidden_dim + input_dim, hidden_dim, press, vres,
                             3)
        self.convq = SConv3d(hidden_dim + input_dim, hidden_dim, press, vres,
                             3)
