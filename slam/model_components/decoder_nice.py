...
from slam.common.common import normalize_3d_coordinate
...
class MLP(nn.Module):
    ...
    def __init__(self,
        ...
        if pos_embedding_method == 'fourier':
            ...
            if 'color' in name:
                multires = 10
                self.embedder = Nerf_positional_embedding(multires,
                                                          log_sampling=True)
            else:
                multires = 5
                self.embedder = Nerf_positional_embedding(multires,
        ...
        self.pts_linears = nn.ModuleList([
            ...
            self.skips else DenseLayer(
                hidden_size + embedding_size, hidden_size, activation='relu')
            for i in range(n_blocks - 1)
        ])

        if self.color:
            self.output_linear = DenseLayer(hidden_size,
                                            4,
                                            activation='linear')
        else:
            self.output_linear = DenseLayer(hidden_size,
...
class MLP_no_xyz(nn.Module):
    """Decoder. Point coordinates only used in sampling the feature grids, not
    as MLP input.
    ...
    def __init__(self,
        ...
        self.pts_linears = nn.ModuleList(
            [DenseLayer(hidden_size, hidden_size, activation='relu')] + [
                ...
                DenseLayer(hidden_size + c_dim, hidden_size, activation='relu')
                for i in range(n_blocks - 1)
            ])
...
class NICE(nn.Module):
    ...
    def __init__(self,
        ...
        if coarse:
            self.coarse_decoder = MLP_no_xyz(name='coarse',
                                             dim=dim,
                                             c_dim=c_dim,
                                             color=False,
